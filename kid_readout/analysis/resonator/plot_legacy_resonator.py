"""
This is a plotting module for the legacy_resonator code.
"""
...
def extract(r, normalize_s21=False, freq_scale=1, points=1000):
...
def amplitude(r, title="", xlabel='frequency [MHz]', ylabel='$|S_{21}|^2$ [dB]', plot_masked=True, **kwds):
    """
    ...
    _plot_amplitude_on_axis(extracted, axis)
...
def IQ_circle(r, title="", xlabel=r"Re $S_{21}$", ylabel=r"Im $S_{21}$", plot_masked=True, **kwds):
    ...
    if plot_masked and extracted['masked'].size:
        ax.plot(extracted['masked'].real, extracted['masked'].imag, linestyle='None', marker='.', color='gray',
                label='masked')
    ax.plot(extracted['model'].real, extracted['model'].imag, linestyle='-', linewidth=0.5, color='brown', label='fit')
    ax.plot(extracted['model_0'].real, extracted['model_0'].imag, linestyle='None', marker='.', color='brown',
            label='$f_0$')
...
def five_by_four(resonators, title="", xlabel='frequency [MHz]', ylabel='$|S_{21}|$ [dB]', sort=False, **kwds):
    ...
    for n, r in enumerate(resonators):
        ...
        _plot_amplitude_on_axis(e, axis)
        axis.set_xlabel("")
        axis.set_ylabel("")
        ...
        axis.set_xticks(xticks)
        axis.set_xticklabels(['{:.3f}'.format(tick) for tick in xticks])
        yticks = [20 * np.log10(np.abs(e['model_0'])),
                  20 * np.log10(np.mean((np.abs(e['data'][0]), np.abs(e['data'][-1]))))]
...
def covar(mzr, nsigma=8, nx=15, figsize=(8, 8)):
    ...
    for p1 in range(nparam):
        for p2 in range(p1 + 1, nparam):
            ...
            x, y, im = lmfit.conf_interval2d(mzr, pnames[p1], pnames[p2],
                                             limits=((x0 - xerr, x0 + xerr), (y0 - yerr, y0 + yerr)), nx=nx, ny=nx)
