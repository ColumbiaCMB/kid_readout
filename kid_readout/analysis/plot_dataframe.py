...
def plot_responsivity(df, nrows=4, ncols=4, figsize=(6, 6), loglog=False, power='zbd_power', fits=True, P_scale=1e6,
                      P_limits=None, X_scale=1e6, X_limits=None, I_scale=1e6, I_limits=None, X_color='blue',
                      I_color='green', P_label='power [$\mu$W]', X_label='$10^6 X$', I_label='$10^6 Q_i^{-1}$'):
    ...
    for plot_index, (X_ax, (channel, group)) in enumerate(zip(axes.flatten(), masked.groupby('channel'))):
        ...
        if plot_index == (nrows - 1) * ncols:
            ...
            X_ax.set_ylabel(X_label)
        if plot_index == (nrows * ncols - 1):
            I_ax.set_ylabel(I_label)
