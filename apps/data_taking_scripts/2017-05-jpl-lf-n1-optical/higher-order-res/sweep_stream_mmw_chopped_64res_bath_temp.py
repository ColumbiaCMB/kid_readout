...
cold_temp = [150,180,210]

for temp in cold_temp:
    raw_input('change bath temp to %3.3f' %temp)
    if temp == 150:
        print "temp is 150, skipping wait"
    else:
        print "waiting 10 minutes", temp
        time.sleep(600)
    ...
    ncf_source_off = new_nc_file(suffix= 'mmw_broadband_source_off_bath_temp_%.3f_mK' % temp)
    ...
    for turn_num in turnlist:
        ...
        ri.set_modulation_output('high')
        ncf = new_nc_file(suffix='mmw_broadband_source_on_%.2f_turns_bath_temp_%.3f_mK' %(turn_num, temp))
