...
for heater_voltage in np.sqrt(np.linspace(0,4**2,4)):
    ...
    ncf_source_off = new_nc_file(suffix= 'mmw_broadband_source_off_load_heater__%.3f_V' % heater_voltage)
    ...
    for turn_num in turnlist:
        ...
        ri.set_modulation_output('high')
        ncf = new_nc_file(suffix='mmw_broadband_source_on_%.2f_turns_load_heater__%.3f_V' %(turn_num, heater_voltage))
