...
inital_f0s = np.load('/data/readout/resonances/2017-05-09-bnl-hex-271-lo-1210-mhz-64.npy')
...
for (lo,f0s) in [(1210.,inital_f0s),]:
    ...
    for sidx in range(swpa.num_channels):
        ...
        if np.abs(f0s[sidx]*1e6-res.f_0) > 300e3:
            current_f0s.append(f0s[sidx]*1e6)
            logger.info("Resonator index %d moved more than 300 kHz, keeping original value %.1f MHz" % (sidx,
                                                                                                         f0s[sidx]))
        else:
