"""
This script doesn't really work because at high power the source shifts the resonance out of the sweep.
"""
...
def main(source_on_resonances, source_off_resonances, mmw_source_frequency, source_on_offset_shift=0,
         suffix="mmw_narrowband_step",
         hittite_power=0, approximate_stream_time=30, coarse_exponent=19, fine_exponent=21,
         integer_modulation_rate=7):

    if mmw_source_frequency:
        ...
        hittite.set_freq(mmw_source_frequency / frequency_multiplication_factor)  # in Hz
        hittite.on()
    ...
    while True:
        mmw_source_modulation_freq = roach.set_modulation_output(modulation_rate)
        ...
        mmw_source_modulation_freq = roach.set_modulation_output(7)
        ...
        for k, attenuation in enumerate(attenuations):
            ...
            source_on_modulation_freq = source_on()
            print("Source on.")
            time.sleep(1)
            on_fit_f0s = sweep_and_stream(df, roach, lockin, f0s.size, approximate_stream_length,
                                          range(fine_offset_frequencies.size), k > 0,  # overwrite after the first sweep.
                                          mmw_source_frequency, source_on_modulation_freq)
            print("on - initial [Hz]: " + ', '.join(['{:.0f}'.format(1e6 * diff) for diff in on_fit_f0s - f0s]))
...
if __name__ == '__main__':
    source_off_resonances = np.load('/home/data2/resonances/2014-12-06_140825_0813f8_fit_16.npy')
    source_on_resonances = source_off_resonances
    mmw_source_frequency = 156e9
    attenuations = [41, 38, 35, 32, 29, 26, 23]
    approximate_stream_length = 30  # in seconds


    main(source_off_resonances, source_on_resonances, mmw_source_frequency, attenuations)