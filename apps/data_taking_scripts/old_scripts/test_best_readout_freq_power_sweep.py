...
f0s = np.load('/home/gjones/workspace/apps/sc5x4_0813f12.npy')#[::3][:4]
f0s.sort()
#f0s = f0s*(0.9995)
...
#    ri.add_tone_freqs(np.array(meas_cfs))
    ...
    nsamp = 2**22
    step = 1
    offset_bins = np.array([-6,-5,-4,-3,-2,-1,0,1,2])#np.arange(-4,4)*step
    offsets = offset_bins*512.0/nsamp
    ...
    for k in range(ri.tone_bins.shape[0]):
        ri.select_bank(k)    
        df.log_hw_state(ri)
    ...
    raw_input ("turn compressor back on")
    df.nc.close()
