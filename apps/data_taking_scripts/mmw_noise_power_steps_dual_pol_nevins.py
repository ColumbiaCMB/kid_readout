    ...
    for k, atten in enumerate(attenlist):
        ...
        for source_state in ['on', 'off']:
            ...
            for iset in range(nsets):
                ...
                tsg = df.add_timestream_data(dmod, ri, t0, tsg=tsg, mmw_source_freq=f_mmw_source,
                                             mmw_source_modulation_freq=mmw_source_modulation_freq,
                                             zbd_voltage=x)
                df.sync()
            print "done with timestream with source " + source_state

        # now we're reading out at frequencies corresponding to the source off resonance
        ...
        for iset in range(nsets):
            ...
            time.sleep(0.4)
            t0 = time.time()
            dmod, addr = ri.get_data(4*16) #ri.get_data_seconds(30)
            x, y, r, theta = lockin.get_data()
            # Record the timestream data taken using the selected tone bank.
            tsg = df.add_timestream_data(dmod, ri, t0, tsg=tsg, mmw_source_freq=f_mmw_source,
                                         mmw_source_modulation_freq=mmw_source_modulation_freq,
                                         zbd_voltage=x)
            df.sync()
        print "done with timestream with source modulated"
