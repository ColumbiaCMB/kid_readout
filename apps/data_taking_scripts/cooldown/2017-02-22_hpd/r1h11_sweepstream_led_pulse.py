...
#f_center = f_center_all[4:6] # select a subset of the frequencies
f_center = 1e6 * np.array([2583.6])
...
generator = hardware.Thing(name='generator', state={'waveform': 'pulse',
                                                    'V_pp': 0,
                                                    'frequency': 100,
                                                    'duty_cycle': 0.01})
resistor = hardware.Thing(name='resistor', state={'resistance_ohms': 47, 'V_pp': 0})
...
try:
    ...
    for lo_index, f_lo in enumerate(f_lo_center):
        ...
        for attenuation_index, (attenuation, fft_gain) in enumerate(zip(attenuations, fft_gains)):
            ...
            vpp_out = float_input("Enable the function generator output and enter the p-p voltage: ")
            vpp_resistor = float_input("Enter the p-p voltage across the resistor: ")
