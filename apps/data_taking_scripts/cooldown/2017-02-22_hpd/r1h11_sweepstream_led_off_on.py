...
attenuations = [30, 36]
fft_gains = [4, 5]
f_center_all = 1e6 * np.array([1991.8,  # no man's land; probably not an MKID
                               2584.1,  # Band 0, lower edge
                               2694.9,  # Band 1,
                               2798.2,  # no-man's land
                               2916.1,  # Band 2
                               2988.1,  # Band 3
                               3864.3,  # no man's land; probably not a MKID
                               ])
...
df_total = 3e6  # The total span of the baseband tones
...
source = hardware.Thing(name='source', state={'current_mA': 0})
hw = hardware.Hardware(conditioner, shield, source)
...
try:
    ...
    for lo_index, f_lo in enumerate(f_lo_center):
        ...
        for attenuation_index, (attenuation, fft_gain) in enumerate(zip(attenuations, fft_gains)):
            ...
            fine_sweep_on = acquire.run_loaded_sweep(ri, length_seconds=sweep_length_seconds,
                                                     tone_bank_indices=fine_indices)[0]
            ri.select_bank(np.argmin(np.abs(f_baseband_bin_center - f_baseband)))
            ri.select_fft_bins(np.array([0]))
            logger.info("Recording {:.1f} s stream".format(stream_length_seconds))
            stream_on = ri.get_measurement(num_seconds=stream_length_seconds, demod=True)[0]
            npd.write(basic.SingleSweepStream(sweep=fine_sweep_on, stream=stream_on, state=state))
            npd.write(ri.get_adc_measurement())
            raw_input("Disable the source output.")
            state['source']['current_mA'] = 0
finally:
    npd.close()
