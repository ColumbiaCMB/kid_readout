...
tic = time.time()
try:
    for band_index, f_center in enumerate(bands):
        ...
        f_dummy = f_baseband_maximum + 2 * df_filterbank * np.arange(1, num_dummy_frequencies + 1)
        ...
        for attenuation_index, attenuation in enumerate(attenuations):
            ...
            fit_f_r = [sweep_array[number].resonator.f_0 for number in range(sweep_array.num_channels)]
            ...
            stream_array = ri.get_measurement(num_seconds=stream_length_seconds, demod=True, state=hw.state())
