...
f0 = all_f0_MHz[3:4]
attenuations = [40]
# Gives 2 ** 4 tones per bin
tone_sample_exponent = 15
# Use all the tones in the bin, including both edges
n_offset = np.arange(-8, 9, 1)
num_tones_sweep = 128
...
tic = time.time()
try:
    ri.set_tone_baseband_freqs(freqs=f_baseband_MHz_sweep[:, np.newaxis], nsamp=2 ** tone_sample_exponent)
    for resonator_index, f_lo_MHz_center in enumerate(f_lo_MHz_centers):
        for attenuation_index, attenuation in enumerate(attenuations):
            ...
            # Do a sweep to find the resonance, then use this value to set the LO for subsequent sweeps.
            for offset_index, f_lo_MHz_offset in enumerate(f_lo_MHz_offsets):
                ...
                sss = basic.SingleSweepStream(sweep=sweep, stream=stream)
                npd.write(sss)
